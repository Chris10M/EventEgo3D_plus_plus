...
from dataset.transforms import camera_to_j2d_batch
...
def compute_fn(model, batch, prev_buffer=None, prev_key=None, batch_first=False):
    ...
    return inps.view(T * B, C, H, W), outputs, gt_hms, gt_j3d, gt_seg, gt_j2d, vis_j2d, vis_j3d, vis_ja, valid_j3d, valid_seg, frame_index
...
def train(config, train_loader, model, criterions, optimizer, epoch, output_dir, tb_log_dir, writer_dict, pretraining=True):
    ...
    j2d_losses = AverageMeter()
    angle_losses = AverageMeter()
    length_losses = AverageMeter()
    acc = AverageMeter()
    ...
    for i, batch in enumerate(train_loader):
        ...
        inp, outputs, gt_hms, gt_j3d, gt_seg, gt_j2d, vis_j2d, vis_j3d, vis_ja, valid_j3d, valid_seg, frame_index = compute_fn(model, batch)
        ...
        pred_j2d = camera_to_j2d_batch(outputs['j3d'], config)        

        gt_j3d = gt_j3d  * 1000 # scale to mm
        pred_j3d = outputs['j3d'] * 1000 # scale to mm        

        loss_hms = criterions['hms'](pred_hms, gt_hms, vis_j2d * 20)  # scale to 20
        loss_seg = criterions['seg'](pred_seg, gt_seg, valid_seg) * 1e-1 # scale to 1e-1
        
        loss_j3d = criterions['j3d'](pred_j3d, gt_j3d, vis_j3d * 1e-2)  # scale to 1e-2
        
        loss_j2d = criterions['j2d'](pred_j2d, gt_j2d, vis_j2d * 1e-2) # scale to 1e-2
        loss_angle, loss_length = criterions['bone'](pred_j3d, gt_j3d, vis_ja, vis_ja * 1e-3) 


        loss = loss_hms + loss_j3d + loss_seg + loss_j2d + loss_angle + loss_length
        ...
        j2d_losses.update(loss_j2d.item(), inp.size(0))
        angle_losses.update(loss_angle.item(), inp.size(0))
        length_losses.update(loss_length.item(), inp.size(0))
        ...
        if i % config.PRINT_FREQ == 0:
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                ...
                'ANGLE_Loss {angle_loss.val:.5f} ({angle_loss.avg:.5f})\t' \
                'LENGTH_Loss {length_loss.val:.5f} ({length_loss.avg:.5f})\t' \
                ...
                'MPJPE {acc.val:.3f} ({acc.avg:.3f})'.format(
                    ...
                    hms_loss=hms_losses,
                    seg_loss=seg_losses,
                    angle_loss=angle_losses,
                    length_loss=length_losses,
                    j2d_loss=j2d_losses,
                    acc=acc
                    )
            logger.info(msg)
...
def validate(config, val_loader, val_dataset, model, criterions, output_dir, tb_log_dir, writer_dict):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):            
            inp, outputs, gt_hms, gt_j3d, gt_seg, gt_j2d, vis_j2d, vis_j3d, vis_ja, valid_j3d, valid_seg, frame_index = compute_fn(model, [batch], buffer, key, batch_first=True)                    
            ...
            preds_j2d = camera_to_j2d_batch(outputs['j3d'], config).cpu().numpy()        
