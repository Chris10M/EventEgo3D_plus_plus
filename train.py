...
from core.loss import HeatMapJointsMSELoss, J3dMSELoss, SegmentationLoss, BoneLoss
...
def main():
    ...
    hms_criterion = HeatMapJointsMSELoss(use_target_weight=cfg.LOSS.USE_TARGET_WEIGHT).cuda()
    j3d_criterion = J3dMSELoss(use_target_weight=cfg.LOSS.USE_TARGET_WEIGHT).cuda()
    j2d_criterion = J3dMSELoss(use_target_weight=cfg.LOSS.USE_TARGET_WEIGHT).cuda()

    criterions = {}
    criterions['hms'] = hms_criterion
    criterions['j3d'] = j3d_criterion
    criterions['j2d'] = j2d_criterion
    criterions['seg'] = SegmentationLoss().cuda()
    criterions['bone'] = BoneLoss().cuda()
